...
public:
	SDCARD();
	~SDCARD();
	void setupHw(SPI_TypeDef* spi, uint16_t spiPrescaler, GPIO_TypeDef* controlPort, uint16_t csPin);
	SD_Error init(void);
	uint8_t detect(void);
	SD_Error getCardInfo(SD_CardInfo *cardinfo);
	SD_Error readBlock(uint8_t* pBuffer, uint32_t ReadAddr, uint16_t BlockSize);
	SD_Error readMultiBlocks(uint8_t* pBuffer, uint32_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks);
	SD_Error writeBlock(uint8_t* pBuffer, uint32_t WriteAddr, uint16_t BlockSize);
	SD_Error writeMultiBlocks(uint8_t* pBuffer, uint32_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks);
...
private:
	...
	uint8_t getDataResponse(void);
	SD_Error goIdleState(void);
