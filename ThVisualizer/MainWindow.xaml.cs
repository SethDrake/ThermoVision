...
namespace ThVisualizer
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private static Color[] DEFAULT_COLOR_SCHEME = new Color[]{
        ...
        private static Color[] ALTERNATE_COLOR_SCHEME = new Color[]{
        ...
        private void drawThermalMap(float scale)
        {
            ...
            int imageWidth = (int)Math.Round(mapWidth * 2 * scale);
            int imageHeight = (int)Math.Round(mapHeight * scale);
            Bitmap bitmap = new Bitmap(mapWidth * 2, mapHeight, PixelFormat.Format24bppRgb);
            int i = 0;
            if (thermalMap.ColorMap.Count > 0)
            {
                for (int y = 0; y < mapHeight; y++)
                {
                    for (int x = 0; x < mapWidth; x++)
                    {
                        ...
                        bitmap.SetPixel(x, mapHeight - y - 1, rgb565ToColor(thermalMap.ColorMap[i]));
                        i++;
                    }
                }
            }
            i = 0;
            if (thermalMap.PhotoMap.Count > 0)
            {
                for (int y = 0; y < mapHeight; y++)
                {
                    for (int x = mapWidth; x < mapWidth * 2; x++)
                    {
                        ...
                        bitmap.SetPixel(x, mapHeight - y - 1, rgb565ToColor(thermalMap.PhotoMap[i]));
                        i++;
                    }
                }
            }
